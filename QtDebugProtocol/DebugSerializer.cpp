...
void DebugDBSerializer::serializeObject(const std::string_view& a_name, const std::string_view& a_classname, const uint64_t& a_objUID, const uint64_t& a_owner)
{
	m_objList.append(DebugProtocol::writeObjectInfo(a_name, a_classname, a_objUID, a_owner));
}


void DebugSerializer::beginObject(const std::string_view& a_name, const std::string_view& a_classname, const uint64_t& a_objUID, const uint64_t& a_owner)
{
	m_currentObjName = a_name;
	m_currentObjOwner = a_owner;
	m_currentUID = a_objUID;
	m_currentClassName = a_classname;
}
...
{
	m_containerObject = DebugProtocol::writeObject(m_currentObjName, m_currentClassName, m_currentUID, m_currentObjOwner, m_propList);
}
